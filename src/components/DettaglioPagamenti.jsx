...
export default function DettaglioPagamenti({ user }) {
  ...
  const loadDettagli = async () => {
    ...
    try {
      ...
      const currentQuota = Number(annoData?.quota_iscrizione || 0);
      setQuotaIscrizione(currentQuota);

      // A. RECUPERO TARIFFE 
      const { data: tariffeData } = await supabase.from('tariffe').select('tipo_lezione, costo').eq('anno_accademico', selectedAnno).eq('school_id', schoolIdTarget);
      const costiMap = {};
      tariffeData?.forEach(t => { costiMap[t.tipo_lezione] = Number(t.costo); });

      // B. RECUPERO PACCHETTI (SCONTI)
      const { data: pacchettiData } = await supabase.from('tariffe_pacchetti').select('*').eq('anno_accademico', selectedAnno).eq('school_id', schoolIdTarget);
      ...
      const myAlunniIds = assocData?.map(row => row.alunno_id) || [];
      ...
      // D. RECUPERO LEZIONI GLOBALI PER GLI ALUNNI SELEZIONATI
      if (myAlunniIds.length > 0) {
          const { data: lezioniRaw, error: lezError } = await supabase.from('registro')
            .select(`id, alunno_id, docente_id, data_lezione, importo_saldato, tipi_lezioni!inner ( tipo ), alunni ( id, nome, cognome ), school_id`)
            .in('alunno_id', myAlunniIds)
            .gte('data_lezione', startDateBuffer).lte('data_lezione', endDateBuffer);
          ...
          processedEvents.forEach(evt => {
            const alu = alunniMap[evt.alunno_id];
            if (!alu) return;

            const mese = new Date(evt.data_lezione).getMonth() + 1;
            const tipoLezione = evt.tipi_lezioni?.tipo || 'Altro';
            
            // Forziamo il calcolo matematico
            const costo = evt.is_virtual ? Number(evt.costo_calcolato) : Number(costiMap[tipoLezione] || 0);
            const saldato = Number(evt.importo_saldato) || 0;

            // 1. AGGIORNAMENTO TOTALI GLOBALI ALUNNO (Sempre, per non perdere gli sconti)
            if (alu.mesi[mese]) {
                alu.mesi[mese].dovuto = Number((alu.mesi[mese].dovuto + costo).toFixed(2));
                alu.mesi[mese].pagato = Number((alu.mesi[mese].pagato + saldato).toFixed(2));
            }

            // 2. FILTRO PER SOTTORIGHE (Visualizziamo solo ciò che appartiene al docente selezionato)
            if (evt.docente_id === selectedDocenteId) {
                if (!alu.tipologie[tipoLezione]) {
                    alu.tipologie[tipoLezione] = { mesi: createEmptyMonths(), totale: {dovuto:0, pagato:0} };
                }
                const tMese = alu.tipologie[tipoLezione].mesi[mese];
                if (tMese) {
                    tMese.dovuto = Number((tMese.dovuto + costo).toFixed(2));
                    tMese.pagato = Number((tMese.pagato + saldato).toFixed(2));
                }
            }
          });
      }
      ...
      if (alunniIds.length > 0) {
          const { data: pagamentiIscrizione } = await supabase.from('pagamenti').select('alunno_id, importo').in('alunno_id', alunniIds).eq('anno_accademico', selectedAnno).eq('tipologia', 'Iscrizione');

          pagamentiIscrizione?.forEach(pay => {
              if (alunniMap[pay.alunno_id]) {
                  const val = Number(pay.importo);
                  if(alunniMap[pay.alunno_id].mesi[0]) {
                      alunniMap[pay.alunno_id].mesi[0].pagato = Number((alunniMap[pay.alunno_id].mesi[0].pagato + val).toFixed(2));
                  }
                  ...
                  alunniMap[pay.alunno_id].tipologie['Iscrizione'].mesi[0].pagato = Number((alunniMap[pay.alunno_id].tipologie['Iscrizione'].mesi[0].pagato + val).toFixed(2));
              }
          });
      }
      ...
      Object.values(alunniMap).forEach(alu => {
         alu.mesi[0].dovuto = currentQuota; 
         ...
         alu.totale = { dovuto: Number(totD.toFixed(2)), pagato: Number(totP.toFixed(2)) };
         ...
         Object.values(alu.tipologie).forEach(tipo => {
             ...
             tipo.totale = { dovuto: Number(tD.toFixed(2)), pagato: Number(tP.toFixed(2)) };
         });
      });

      setMatrix(Object.values(alunniMap).sort((a, b) => a.nome_completo.localeCompare(b.nome_completo)));
    } catch (err) {
  ...
  const handleCellClick = async (alunnoId, mese, tipo) => {
    ...
    try {
      ...
      const bufferStart = new Date(firstDayOfMonth); bufferStart.setDate(bufferStart.getDate() - 10);
      const bufferEnd = new Date(lastDayOfMonth); bufferEnd.setDate(bufferEnd.getDate() + 10);
      ...
      if (tipo === 'dovuto') {
        if (mese === 0) {
          items = [{ data: '-', desc: 'Quota Iscrizione', importo: quotaIscrizione }];
            ...
            const { data: tData } = await supabase.from('tariffe').select('tipo_lezione, costo').eq('anno_accademico', selectedAnno).eq('school_id', user?.school_id);
            const mapCosti = {}; tData?.forEach(t => mapCosti[t.tipo_lezione] = Number(t.costo));

            const { data: lezRaw } = await supabase.from('registro')
                .select(`id, alunno_id, docente_id, data_lezione, tipi_lezioni(tipo), school_id`)
                .eq('alunno_id', alunnoId).gte('data_lezione', bufferStart.toISOString().split('T')[0]).lte('data_lezione', bufferEnd.toISOString().split('T')[0]);

            items = calcolaScontiPacchetti(lezRaw || [], pacchettiRules)
                .filter(item => {
                    const itemDate = new Date(item.data_lezione);
                    return (itemDate.getMonth() + 1) === mese && item.docente_id === selectedDocenteId;
                })
                .map(l => ({
                    data: new Date(l.data_lezione).toLocaleDateString('it-IT'),
                    desc: l.is_virtual ? l.descrizione_sconto : (l.tipi_lezioni?.tipo || 'Lezione'),
                    note: l.is_virtual ? l.dettaglio_settimana : '',
                    importo: l.is_virtual ? Number(l.costo_calcolato) : (mapCosti[l.tipi_lezioni?.tipo] || 0),
                    is_discount: l.is_virtual
                }));
        }
      } else {
        if (mese === 0) {
          ...
          if (ids.length > 0) {
            const { data: det } = await supabase.from('dettagli_pagamento').select(`importo_coperto, pagamenti(data_pagamento, metodo_pagamento, tipologia, note)`).in('registro_id', ids);
            items = (det || []).map(d => ({ data: new Date(d.pagamenti.data_pagamento).toLocaleDateString('it-IT'), desc: `${d.pagamenti.tipologia} (${d.pagamenti.metodo_pagamento})`, note: d.pagamenti.note, importo: d.importo_coperto }));
          }
        }
      }
      setDetailData({ title, items, modalType: tipo });
    } catch (e) { setDetailData({ title: 'Errore', items: [], modalType: tipo }); }
  };
  ...
  const renderCellContent = (cell, alunnoId, meseVal, isClickable = true) => {
      ...
      return (
        <div className="flex flex-col justify-center items-center gap-0.5 w-full h-full text-xs">
            <div className={`flex justify-between w-full px-1 text-gray-500 rounded ${isClickable ? "cursor-pointer hover:bg-white/10 transition-colors" : "opacity-80"}`} onClick={() => isClickable && handleCellClick(alunnoId, meseVal, 'dovuto')}>
                <span>D:</span> <span className="text-gray-300">€ {cell.dovuto.toFixed(1)}</span>
            </div>
            <div className={`flex justify-between w-full px-1 text-gray-500 rounded ${isClickable ? "cursor-pointer hover:bg-white/10 transition-colors" : "opacity-80"}`} onClick={() => isClickable && handleCellClick(alunnoId, meseVal, 'pagato')}>
                <span>P:</span> <span className="text-white">€ {cell.pagato.toFixed(1)}</span>
            </div>
  ...
  const filteredMatrix = matrix.filter(row => row.nome_completo.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="flex flex-col h-full bg-accademia-card border border-gray-700 rounded-xl overflow-hidden shadow-xl">
      <div className="px-4 py-3 border-b border-gray-700 bg-gray-900/30 flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0">
        <div className="flex items-center gap-2"><Users className="text-accademia-red" size={20}/><h2 className="text-lg font-light text-white">Riepilogo Pagamenti</h2></div>
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <div className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-1 border border-gray-600">
                <Search size={14} className="text-gray-400" /><input type="text" placeholder="Filtra alunno..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-transparent text-white text-sm focus:outline-none placeholder-gray-500"/>
                {searchTerm && <button onClick={() => setSearchTerm('')} className="text-gray-500 hover:text-white"><X size={14} /></button>}
            </div>
            <div className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-1 border border-gray-600">
                <Filter size={14} className="text-gray-400"/><select value={selectedAnno} onChange={(e) => setSelectedAnno(e.target.value)} className="bg-transparent text-white text-sm font-bold focus:outline-none cursor-pointer">
                    {ANNI_ACCADEMICI.map(anno => <option key={anno} value={anno} className="bg-gray-800 text-white">{anno}</option>)}
                </select>
            </div>
            <select className="bg-accademia-input border border-gray-700 text-white rounded-lg px-4 py-2 w-full sm:w-64 focus:border-accademia-red focus:outline-none" value={selectedDocenteId} onChange={(e) => setSelectedDocenteId(e.target.value)}>
      ...
      <div className="flex-1 overflow-auto p-0 custom-scrollbar relative">
        {!selectedDocenteId ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-500"><Users size={48} className="opacity-20 mb-2" /><p>Seleziona un docente per visualizzare il prospetto.</p></div>
        ) : loading ? (
            ...
            <thead className="bg-accademia-red text-white sticky top-0 z-30 shadow-md uppercase tracking-wider font-bold">
              <tr>
                ...
                {COLONNE_RENDER.map(m => (
                  <th key={m.val} className={`p-2 min-w-[75px] border-r border-red-800 text-xs ${m.val===0 ? 'bg-red-900 border-red-700' : ''}`}>{m.label}</th>
                ))}
                    ...
                    <tr className="hover:bg-gray-800/30 transition-colors group border-b border-gray-700 relative z-20">
                      <td className="p-0 border-r border-gray-700 sticky left-0 bg-accademia-card group-hover:bg-gray-800 z-10 shadow-r-lg">
                        <div className="flex items-center h-16 px-2 w-full">
                          {Object.keys(row.tipologie).length > 0 && (
                            <button onClick={() => toggleRow(row.id)} className="mr-2 p-1 rounded hover:bg-gray-700 text-gray-400 hover:text-white">{expandedRows[row.id] ? <ChevronDown size={16} /> : <ChevronRight size={16} />}</button>
                          )}
                      ...
                      <td className="p-2 border-r border-gray-700 bg-gray-900/40 text-center align-middle">
                        <div className="flex flex-col text-xs">
                          <div className="text-gray-400">D: € {row.totale.dovuto.toFixed(1)}</div>
                          <div className="text-white">P: € {row.totale.pagato.toFixed(1)}</div>
                    ...
                    {expandedRows[row.id] && Object.entries(row.tipologie).map(([tipo, datiTipo]) => (
                      <tr key={`subrow-${row.id}-${tipo}`} className="bg-gray-800/40 border-b border-gray-700/50 animate-in fade-in slide-in-from-top-1 duration-200">
                        ...
                        <td className="p-2 border-r border-gray-700/50 bg-gray-900/20 text-center align-middle h-16">
                          <div className="flex flex-col text-xs">
                            <div className="text-gray-400">D: € {datiTipo.totale.dovuto.toFixed(1)}</div>
                            <div className="text-white">P: € {datiTipo.totale.pagato.toFixed(1)}</div>
                          </div>
                        </td>
